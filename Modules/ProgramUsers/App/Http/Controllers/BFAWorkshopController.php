...
class BFAWorkshopController extends Controller
{
    public function customers(Request $request)
    {
        if ($request->ajax()) {
            ...
            $query = Customer::select(
                ...
                ->where('is_user', 1)
                ->where('is_delete', 0)
                ->where('product_id', config('constant.BODYFAT_ANALYSIS_WORKSHOP_ID'));
            ...
            return DataTables::of($query)
                ...
                ->addColumn('action', function ($row) {
                    $actionBtn = '<a class="" href="' . route('manage.bodyfat-analysis-workshop.customers.details', ['userId' => $row->id]) . '"><i class="fa fa-info-circle"></i></a>';
                    return $actionBtn;
                })
                ->rawColumns(['date', 'fullname', 'action'])
                ->make(true);
        }
        return view('programusers::bodyfat-analysis-workshop.customers');
    }
    ...
    public function usersDetails($userId)
    {
        ...
        if ($customer != null) {
            return view('programusers::bodyfat-analysis-workshop.customerDetails', compact(['customer', 'invoices', 'referralUsers', 'diseases']));
        } else {
    ...
    public function usersDetailsUpdate(Request $request)
    {
        ...
        $request->validate([
            'first_name' => 'required',
            'last_name' => 'required',
            // 'email' => 'required|email|unique:customers,email,' . $userId . ',id,product_id,' . config('constant.BODYFAT_ANALYSIS_WORKSHOP_ID'),
    ...
    public function downloadBMIReport($userId)
    {
        ...
        $product = Product::find(config('constant.BODYFAT_ANALYSIS_WORKSHOP_ID'));
        $url = "https://fittoss.com/bodyfat-analysis-workshop";
    ...
    public function statistics()
    {
        ...
        $otps = OtpVerification::where('product_id', config('constant.BODYFAT_ANALYSIS_WORKSHOP_ID'))
        ...
        return view('programusers::bodyfat-analysis-workshop.statistics', compact('leads', 'customers', 'amount', 'otps'));
    }
    ...
    public function leads(Request $request)
    {
        ...
        return view('programusers::bodyfat-analysis-workshop.leads');
    }
    ...
    public function info(Request $request)
    {
        ...
        $rec['otps'] = $userOtps;
        return view('programusers::bodyfat-analysis-workshop.infodetails')->with($rec);
    }
