...
class WLPCustomersController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($query)
                ...
                ->addColumn('action', function ($row) {
                    $actionBtn = '<a class="" href="' . route('manage.weight-loss-program.customers.details', ['userId' => $row->id]) . '"><i class="fa fa-info-circle"></i></a>';
                    return $actionBtn;
                })
                ->rawColumns(['date', 'fullname', 'action'])
                ->make(true);
        }
        return view('programusers::weight-loss-program.customers');
    }
    ...
    public function usersDetails($userId)
    {
        ...
        if ($customer != null) {
            return view('programusers::weight-loss-program.customerDetails', compact(['customer', 'invoices', 'referralUsers', 'diseases']));
        } else {
    ...
    public function downloadBMIReport($userId)
    {
        ...
        $product = Product::find(config('constant.WEIGHT_LOSS_PROGRAM_ID'));
        $url = "https://fittoss.com/weight-loss-program";
        return view('programusers::weight-loss-program.bmi_detail', compact('userData', 'personalDetails', 'product', 'url'));
        $pdf = Pdf::loadView('programusers::weight-loss-program.bmi_report_pdf', compact('userData', 'personalDetails', 'product'))
