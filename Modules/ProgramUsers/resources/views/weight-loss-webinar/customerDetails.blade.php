...
<h3>Weight Loss Webinar - Customer Details</h3>
@endsection
...
<div class="container-fluid">
    <div class="row g-3">
                ...
                <div class="card-body">
                    <div class="tab-content" id="ver-pills-tabContent">
                        <div class="tab-pane fade show active" id="pills-warning-customerInfo" role="tabpanel" aria-labelledby="pills-warning-customerInfo-tab">
                            <form action="{{ route('manage.weight-loss-webinar.customers.update') }}" method="post" class="customer-update-form" id="customer-update-form">
                                ...
                                <div class="col-md-12 mt-3 d-flex justify-content-between">
                                    <a href="{{ route('manage.weight-loss-webinar.customers.bmi.pdf',$customer->id) }}" target="_blank" class="btn btn-outline-info">Download BMI Report</a>
                                        ...
                                        <table class="table table-bordered plan-table" style="width:100%">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('manage.weight-loss-webinar.customers.invoice.pdf', ['id' => $invoice->id]) }}" target="_blank" class="">
        ...
        <div class="tab-pane fade" id="pills-warning-actions" role="tabpanel" aria-labelledby="pills-warning-actions-tab">
            <form class="row g-3 needs-validation custom-input update-password-form" id="update-password-form" novalidate="" method="post" action="{{ route('manage.weight-loss-webinar.customers.update.password') }}">
...
<script>
    ...
    function deactivate(userid, status) {
        ...
        , }).then((performYes) => {
            if (performYes) {
                $.ajax({
                    url: `{{ route('manage.weight-loss-webinar.customers.deactivate.account') }}`
                    , type: `POST`
                    , data: JSON.stringify({
                        userid: userid
                        , status: status
    ...
    function destroy(userid) {
        ...
        , }).then((performYes) => {
            if (performYes) {
                $.ajax({
                    url: `{{ route('manage.weight-loss-webinar.customers.delete.account') }}`
                    , type: `POST`
                    , data: JSON.stringify({
                        userid: userid
                    })
                    ...
                    , success: function(result) {
                        if (result.type === 'SUCCESS') {
                            toastr.success(result.message);
                            window.location.href = `{{ route('manage.weight-loss-webinar.customers') }}`;
                        } else {
                            toastr.error(result.message);
                        }
                    }
