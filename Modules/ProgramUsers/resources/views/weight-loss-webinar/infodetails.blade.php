...
<div class="modal fade" id="convertCustomerModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="exampleModalLabel1" aria-hidden="true" style="backdrop-filter: brightness(0.7) blur(2px);">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form method="post" action="{{ route('manage.weight-loss-webinar.leads.convertcustomer') }}" class="convert-customer-form" id="convert-customer-form">
        ...
        $("#block-customer").on('click', function() {
            ...
            , }).then((willBlock) => {
                if (willBlock) {
                    ...
                    $.ajax({
                        url: '{!! route("manage.weight-loss-webinar.leads.block.user")  !!}'
                        , type: 'POST'
                        , data: JSON.stringify({
                            id: userId
                        })
        ...
        $("#stop-remarketing").on('click', function() {
            ...
            , }).then((willDND) => {
                if (willDND) {
                    let userId = $(this).data('userid');
                    $.ajax({
                        url: '{!! route("manage.weight-loss-webinar.leads.dnd.user")  !!}'
                        , type: 'POST'
                        , data: JSON.stringify({
                            id: userId
                        })
        ...
        $("#delete-customer").on('click', function() {
            ...
            , }).then((willDelete) => {
                if (willDelete) {
                    let userId = $(this).data('userid');
                    $.ajax({
                        url: '{!! route("manage.weight-loss-webinar.leads.destroy.user")  !!}'
                        , type: 'POST'
                        , data: JSON.stringify({
                            id: userId
                        })
                        ...
                        , success: function(result) {
                            if (result.type === 'SUCCESS') {
                                toastr.success(result.message);
                                $("#infoModals").modal('hide');
                                $("#WLWLeadsTable").DataTable().ajax.reload();
                            } else {
