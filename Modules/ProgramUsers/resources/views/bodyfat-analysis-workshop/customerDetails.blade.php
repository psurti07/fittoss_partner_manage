...
<h3>Bodyfat Analysis Workshop - Customer Details</h3>
@endsection
...
<div class="container-fluid">
    <div class="row g-3">
                ...
                <div class="card-body">
                    <div class="tab-content" id="ver-pills-tabContent">
                        <div class="tab-pane fade show active" id="pills-warning-customerInfo" role="tabpanel" aria-labelledby="pills-warning-customerInfo-tab">
                            <form action="{{ route('manage.bodyfat-analysis-workshop.customers.update') }}" method="post" class="customer-update-form" id="customer-update-form">
                                ...
                                <div class="row mb-2">
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <select name="gender" id="gender" class="form-select">
                                            ...
                                            <option value="1" {{ $customer->personalDetails->gender == 1 ? 'selected' :'' }}>Male</option>
                                            <option value="2" {{ $customer->personalDetails->gender == 2 ? 'selected' :'' }}>Female</option>
                                            <option value="3" {{ $customer->personalDetails->gender == 3 ? 'selected' :'' }}>Other</option>
                                        </select>
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <input type="number" class="form-control" name="age" id="age" value="{{ optional($customer->personalDetails)->age }}" max="{{ date('Y-m-d') }}">
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <select name="active_rate" id="active_rate" class="form-select">
                                            ...
                                            <option value="0" {{ $customer->personalDetails->active_rate == 0 ? 'selected' :'' }}>Little or no activity</option>
                                            <option value="1" {{ $customer->personalDetails->active_rate == 1 ? 'selected' :'' }}>Lightly active</option>
                                            <option value="2" {{ $customer->personalDetails->active_rate == 2 ? 'selected' :'' }}>Moderately active</option>
                                            <option value="3" {{ $customer->personalDetails->active_rate == 3 ? 'selected' :'' }}>Very active</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="row mb-2">
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <input type="number" class="form-control" name="height" id="height" value="{{ optional($customer->personalDetails)->height }}">
                                    ...
                                    <div class="form-group col-md-4">
                                        ...
                                        <input type="number" class="form-control" name="weight" id="weight" value="{{ optional($customer->personalDetails)->weight }}">
                                        @component('components.ajax-error', ['field' => 'weight'])
                                        @endcomponent
                                    </div>
                                    {{-- {{dd(json_decode($customer->personalDetails->medical_issue,true))}} --}}
                                    {{-- medical issue --}}
                                    @php
                                    $medical_issues = ($customer->personalDetails->medical_issue) ? json_decode($customer->personalDetails->medical_issue,true) : [];
                                    @endphp
                                    <div class="form-group col-md-4">
                                        <label for="medical_issue">Medical Issue</label>
                                        <select name="medical_issue[]" id="medical_issue" multiple class="form-select form-control">
                                            @foreach($diseases as $disease)
                                            <option value="{{ $disease->name }}" {{ in_array($disease->name,$medical_issues) ? 'selected' : '' }}>
                                                {{ $disease->name }}
                                            </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-12 mt-3 d-flex justify-content-between">
                                    <a href="{{ route('manage.bodyfat-analysis-workshop.customers.bmi.pdf',$customer->id) }}" target="_blank" class="btn btn-outline-info">Download BMI Report</a>
                                        ...
                                        <table class="table table-bordered plan-table" style="width:100%">
                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('manage.bodyfat-analysis-workshop.customers.invoice.pdf', ['id' => $invoice->id]) }}" target="_blank" class="">
        ...
        <div class="tab-pane fade" id="pills-warning-actions" role="tabpanel" aria-labelledby="pills-warning-actions-tab">
            <form class="row g-3 needs-validation custom-input update-password-form" id="update-password-form" novalidate="" method="post" action="{{ route('manage.bodyfat-analysis-workshop.customers.update.password') }}">
...
<script>
    ...
    function deactivate(userid, status) {
        ...
        , }).then((performYes) => {
            if (performYes) {
                $.ajax({
                    url: `{{ route('manage.bodyfat-analysis-workshop.customers.deactivate.account') }}`
                    , type: `POST`
                    , data: JSON.stringify({
                        userid: userid
                        , status: status
    ...
    function destroy(userid) {
        ...
        , }).then((performYes) => {
            if (performYes) {
                $.ajax({
                    url: `{{ route('manage.bodyfat-analysis-workshop.customers.delete.account') }}`
                    , type: `POST`
                    , data: JSON.stringify({
                        userid: userid
                    })
                    ...
                    , success: function(result) {
                        if (result.type === 'SUCCESS') {
                            toastr.success(result.message);
                            window.location.href = `{{ route('manage.bodyfat-analysis-workshop.customers') }}`;
                        } else {
                            toastr.error(result.message);
                        }
                    }
