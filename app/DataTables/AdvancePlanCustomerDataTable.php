...
class AdvancePlanCustomerDataTable extends DataTable
{
    /**
    ...
    public function query(Customer $model): QueryBuilder
    {
        return $model->newQuery()
            ->paid()
            ->where('is_delete', 0)
            ->where('product_id', config('constant.ADVANCE_PLAN_OFFER_ID'));
    }
    ...
    {
        return $this->builder()
            ->setTableId('advancePlanCustomerTable')
    ...
    {
        return 'AdvancePlanCustomer_' . date('YmdHis');
    }
}
