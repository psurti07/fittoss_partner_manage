...
class FitoneLeadDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($row) {
                ...
                return $actionBtn;
                // $actionBtn = '<a class="" href="' . route('manage.fitone.customer.details', ['userId' => $row->id]) . '"><i class="fa fa-info-circle"></i></a>';
                // return $actionBtn;
            })
            ...
            //     $statusClass = $row->is_paid == 0 ? 'btn-outline-success' : 'btn-outline-info';
            //     $statusText = $row->is_paid == 0 ? 'Lead' : 'Customer';
            //     return '<span class="btn btn-xs ' . $statusClass . '" style="cursor:pointer;" onclick="toggleStatus(' . $row->id . ')">' . $statusText . '</span>';
            // })
    ...
    {
        return $model->newQuery()->unPaid()
            ->where('is_delete', 0)
            ->where('product_id', config('constant.FITONE_OFFER_ID'));
    }
    ...
    {
        return 'FitoneLead_' . date('YmdHis');
    }
}
