...
class AssociatePartnerCustomerDataTable extends DataTable
{
    /**
    ...
    public function query(Customer $model): QueryBuilder
    {
        return $model->newQuery()
            ->paid()
            ->where('is_delete', 0)
            ->where('product_id', config('constant.ASSOCIATE_PARTNER_PROGRAM_OFFER_ID'));
    }
    ...
    {
        return $this->builder()
            ->setTableId('associatePartnerCustomerTable')
    ...
    {
        return 'AssociatePartnerCustomer_' . date('YmdHis');
    }
}
